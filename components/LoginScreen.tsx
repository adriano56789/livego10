...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  useEffect(() => {
    const fetchLastEmail = async () => {
        try {
            ...
            console.warn("Could not fetch last login email. This is expected on first login or if the server is restarting.");
        }
    };
    fetchLastEmail();
  }, []);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FIX: The type of `result` from `api.auth.register` is `{ success: boolean }`, which does not have an `id` property. The check has been updated to use `result.success` to correctly verify a successful registration.
      if (result && result.success) {
        setStatusMessage({ text: 'Conta criada com sucesso! Faça login agora.', type: 'success' });
        setViewMode('login');
        setPassword(''); // Limpa a senha por segurança
      } else {
        // This case might happen if API response changes.
